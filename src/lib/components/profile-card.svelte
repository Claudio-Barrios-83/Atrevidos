<script lang="ts">
  ...
  export let activeAction: 'like' | 'pass' | 'block' | null = null;
  export let actionError = '';

  const dispatch = createEventDispatcher<{
    action: { profile: DiscoverProfile; action: 'like' | 'pass' | 'block' };
  ...
  function dispatchAction(action: 'like' | 'pass' | 'block') {
    ...
    <div class="mt-4 space-y-3">
      ...
      <div class="grid grid-cols-1 gap-2 pt-2 sm:grid-cols-3">
        <button
          ...
          class="inline-flex items-center justify-center rounded-xl border border-gray-300 px-4 py-3 text-sm font-semibold text-gray-700 transition hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-900"
        >
        ...
        <button
          ...
          disabled={actionLoading}
          class="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={actionLoading}
          class="inline-flex items-center justify-center rounded-xl bg-red-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
