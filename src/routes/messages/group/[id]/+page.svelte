...
<div class="min-h-screen bg-gray-50 px-4 py-6 dark:bg-gray-900">
  <div class="mx-auto max-w-4xl space-y-6">
    <header class="flex flex-col gap-4 rounded-3xl bg-white p-5 shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0">
        <a href="/messages" class="text-sm font-medium text-primary-600 transition hover:text-primary-700 dark:text-primary-400">
          ← Volver a mensajes
        </a>
        ...
        {#if conversation}
          <div class="mt-4 flex min-w-0 items-center gap-4">
            ...
            {:else}
              <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-fuchsia-100 text-xl font-bold text-fuchsia-600 dark:bg-fuchsia-950/40 dark:text-fuchsia-300">
                {getInitials(conversation.name)}
              </div>
            {/if}

            <div class="min-w-0">
              <h1 class="truncate text-2xl font-bold text-gray-900 dark:text-white">{conversation.name}</h1>
        ...
        {:else}
          <div class="mt-4">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Grupo</h1>
          </div>
        {/if}
      </div>
    ...
    {#if loading}
      <section class="rounded-3xl bg-white px-6 py-12 text-center shadow-lg dark:bg-gray-800">
        ...
        <p class="mt-4 text-sm text-gray-600 dark:text-gray-400">Cargando grupo…</p>
      </section>
    {:else if loadError}
    ...
    {:else if conversation}
      <section class="space-y-4">
        ...
        >
          {#if messageItems.length === 0}
            <div class="rounded-2xl border border-dashed border-gray-300 px-5 py-8 text-center dark:border-gray-600">
              ...
              <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Rompe el hielo con el primer mensaje del grupo.</p>
            </div>
          {:else}
        ...
        <form class="rounded-3xl bg-white p-5 shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700" on:submit|preventDefault={sendMessage}>
          <label for="group-message-draft" class="text-sm font-semibold text-gray-900 dark:text-white">Escribe un mensaje</label>
          <textarea
            ...
            class="mt-3 w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 shadow-sm outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-200 dark:border-gray-600 dark:bg-gray-900 dark:text-white"
          ></textarea>
